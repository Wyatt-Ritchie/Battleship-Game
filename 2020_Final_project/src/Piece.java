...
public class Piece {
    ...
    public boolean checkValid(int x, int y, Board board) {
        ...
        if (isHorizontal) {
            for (int j = x; j < hitbox.length + x; j++) {
            ...
            for (int k = y; k < hitbox.length + y; k++) {
    ...
    public void placePieceDisplay(int x, int y, Board board) {
        ...
        if(isHorizontal) {  //if the ship is placed horizontal the y coordinate will be constant
            for (int j = x; j < x + hitbox.length; j++) {
                ...
                String fileName = "/Images/ship" + (hitbox.length - 1) + (j + 1 - x) + ".png";
            ...
            for (int k = y; k < y + hitbox.length; k++) {
                ...
                String fileName = "/Images/ship" + (hitbox.length - 1) + (k + 1 - y) + ".png";
    ...
    public void randomizePiecePosition(Board board, boolean isHuman) {
        Rectangle[][] pBoard = board.getBoard();
        int xRand;
        int yRand;
        double rotate;
